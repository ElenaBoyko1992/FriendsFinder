...
Принципы:
1. редьюсеры, АС, TC, экшены, санки и проч. должны храниться все в одном файле
2. Больше уникальности названию экшн-типа (см. Auth-reduser.ts)
3. Async/await вместо .then



сделать тесты (остановилась на user-reduser.test), поправить редьюсеры все, перевести на Async/await вместо .then, 
